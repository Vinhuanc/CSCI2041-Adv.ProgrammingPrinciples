...
let divide_proper (x: int) (y: int) : int option =  (* divides two integers and returns an option containing the result, have case handle when divisor is 0*)
        (* failwith "TODO" *)
        if y = 0 then 
            None 
        else
...
let rec string_of_exercise (e: exercise) : string =
    match e with
        ...
        | Add (e1, e2) -> 
            ("(") ^  string_of_exercise e1 ^ (" + ") ^ string_of_exercise e2 ^(")") 
